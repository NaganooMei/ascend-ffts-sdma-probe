...
- `h2h-registered-mapped-sdma`: registered mapped host source 到 registered mapped host destination。
...
./build/ffts_sdma_probe --device 0 --mode h2h-registered-mapped-sdma --bytes 1048576 --frags 8 --lanes 8 --warmup 1 --repeat 10
```
...
| `--mode` | `all` | `d2d-sdma`、`h2d-sdma`、`h2h-sdma`、`h2h-registered-mapped-sdma` 或 `all`。 |
...
- `h2h-registered-mapped-sdma` 使用 `aclrtMallocHost` 申请 source 和 destination，再分别 `aclrtHostRegisterV2(..., ACL_HOST_REG_MAPPED)` 和 `aclrtHostGetDevicePointer`，FFTS descriptor 使用 mapped source 和 mapped destination。
